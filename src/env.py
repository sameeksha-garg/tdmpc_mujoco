...
# === Gym(nasium) Wrappers ===
# These are used by the non-DMC path, so they can stay at the top level.
...
def make_env(cfg):
    ...
    if len(parts) == 2:
        ...
        try:
            ...
            warnings.warn(
                f"Failed to import dm_control. Assuming '{cfg.task}' is not a DMControl task."
            )
            pass # is_dmc remains False

    if is_dmc:

        # --- LAZY IMPORTS & DEFINITIONS FOR DMCONTROL ---
        # All dm_control and dm_env imports and wrapper classes are
        # defined here, inside the `if is_dmc:` block.
        
        import dm_env
        from typing import Any, NamedTuple
        from dm_control import suite # Re-import for use in this scope
        ...
        env = DefaultDictWrapper(env) # This wrapper is defined at the top level

    else:
        # --- Non-DMC (Gymnasium) Path ---
        assert cfg.modality != 'pixels', \
            "Gym/MuJoCo path currently only supports state observations."

        env = gym.make(cfg.task) # This is gymnasium.make

        env = Float32Obs(env) # Defined at top level
        env = NormalizeAction(env) # Defined at top level
        env = DefaultDictWrapper(env) # Defined at top level
